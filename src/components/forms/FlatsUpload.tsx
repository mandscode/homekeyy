...
export default function FlatsUpload({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      // Filter out flats that already exist in the API

      const existingFlatNumbers = existingFlats?.map((flat: { number: string }) => flat.number.toString());
      const newFlats = jsonData.filter((flat: { flatNo: string }) => !existingFlatNumbers.includes(flat.flatNo.toString()));
